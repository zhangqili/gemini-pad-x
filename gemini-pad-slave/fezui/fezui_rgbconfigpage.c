...
static const char* rgb_global_mode_items[]={"Off", "Individual", "Wave", "Ripple"};
    ...
    lefl_key_attach(&KEY_KNOB, KEY_EVENT_DOWN, LAMBDA(void,(void*k)
    {
        ...
        else
        {
            if(key_selected)
            {
                key_selected = false;
                RGB_Save();
                //Communication_Add8(USART1, PROTOCOL_CMD, CMD_RGB_READ);
                //Communication_USART1_Transmit();
            }
            else
            {
...
lefl_page_t rgbconfigpage={rgbconfigpage_logic,rgbconfigpage_draw,rgbconfigpage_load};
